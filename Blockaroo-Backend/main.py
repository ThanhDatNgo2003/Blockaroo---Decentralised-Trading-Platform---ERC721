...
if "CONTRACT_ADDRESS" not in os.environ:
    ...
    try:
        ...
        query = "INSERT IGNORE INTO TransactionHistory(event, from_address, date, transaction_hash) VALUES (%s, %s, %s, %s);"
        value = ('Contract Creation', admin_address, current_datetime, tx_receipt.transactionHash.hex())
            ...
            for item in items:
                ...
                if(result != None):
                    ...
                    try:
                        ...
                        nonce = w3.eth.get_transaction_count(key['wallet_address'])
                        ...
                        # Format the datetime as a string in the desired format
                        formatted_timestamp = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
                        query = "INSERT IGNORE INTO TransactionHistory(token_id, event, from_address, date, transaction_hash) VALUES (%s, %s, %s, %s, %s);"
                        value = (tokenid, 'Mint', key['wallet_address'], current_datetime, tx_receipt.transactionHash.hex())
                        cursor.execute(query, value)
...
def sellnft(sell: SellModel):
    try:
        ...
        values = (sell.wallet_address,)
        cursor.execute(query, values)
        user_private_key = cursor.fetchone()[0]
        print (os.environ["CONTRACT_ADDRESS"])
        print(sell.wallet_address)
        try:
            price_in_wei = int(sell.price * 10**18)
            
            nonce = w3.eth.get_transaction_count(sell.wallet_address)
            blockaroo_smart_contract = w3.eth.contract(address=os.environ["CONTRACT_ADDRESS"], abi=abi)
            sell_transaction = blockaroo_smart_contract.functions.updatePrice(sell.token_id, price_in_wei, sell.wallet_address).build_transaction(
            {
                "chainId": 1337,
                "gas": 2000000,
                "gasPrice": w3.eth.gas_price,
            ...
            signed_txn = w3.eth.account.sign_transaction(sell_transaction, private_key=user_private_key)
            tx_hash = w3.eth.send_raw_transaction(signed_txn.rawTransaction)
            tx_receipt = None
            while tx_receipt is None:
                    tx_receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
        ...
        return {"message": "'Your NFT has been listed on the marketplace successfully'"}
    except mysql.connector.Error as err:
        return {"error": f"Error: {err}"}
...
def buy_nft(buy: BuyModel):
    try:
        ...
        user_private_key = cursor.fetchone()[0]
        try:
            nonce = w3.eth.get_transaction_count(buy.from_address)
            blockaroo_smart_contract = w3.eth.contract(address=os.environ["CONTRACT_ADDRESS"], abi=abi)
            buy_transaction = blockaroo_smart_contract.functions.transfer(buy.to_address, buy.token_id).build_transaction(
            {
                "chainId": 1337,
                "gas": 2000000,
                "gasPrice": w3.eth.gas_price,
        ...
        # Format the datetime as a string in the desired format
        formatted_timestamp = current_datetime.strftime("%Y-%m-%d %H:%M:%S")
        query = "INSERT IGNORE INTO TransactionHistory(token_id, event, from_address, to_address, date, transaction_hash) VALUES (%s, %s, %s, %s, %s, %s);"
        value = (buy.token_id, 'Transfer', buy.from_address, buy.to_address, current_datetime, tx_receipt.transactionHash.hex())
        cursor.execute(query, value)
